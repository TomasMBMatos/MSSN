...
public class ReynoldsTestApp implements IProcessingApp {
    ...
    public void setup(PApplet p) {
        ...
        BoidN = new Random().nextInt(199);
        boid = flock.getBoid(BoidN);
        // Wander
        wander = new Boid(new PVector(p.random((float)window[0],(float)window[1]), p.random((float)window[2],(float)window[3])),new PVector(),
                    0.5f, 0.5f, p.color(255, 0, 0), p, plt2);
        wander.addBehavior2(new Wander(1f));

        // Pursuer
        pursuer = new Boid(new PVector(p.random((float)window[0],(float)window[1]), p.random((float)window[2],(float)window[3])),new PVector(),
                0.5f, 0.5f, p.color(0, 255, 0), p, plt2);
        pursuer.addBehavior2(new Pursuit(1f));
       ...
       // allTrackingBodies.add(wander);
        ...
        // Seeker
        seeker = new Boid(new PVector(p.random((float)window[0],(float)window[1]), p.random((float)window[2],(float)window[3])),new PVector(),
                0.5f, 0.5f, p.color(0, 0, 255), p, plt3);
        seeker.addBehavior2(new Seek(1f));
        seeker.addBehavior2(new Wander(1f));
        seeker.addBehavior2(new Flee(1f));
