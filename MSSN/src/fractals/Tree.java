...
public class Tree {
    ...
    public Tree(String axiom, Rule[] rules, PVector position, float lengthRef, float angle,
                int nIter, float scalingFactor, float intervalBetweenSeasons, PApplet p) {
        lsys = new LSystem(axiom, rules);
        length = 0;
        growthRate = lengthRef/intervalBetweenSeasons;
    ...
    public void grow(float dt) {
        ...
        else if(lsys.getGen() < numberOfSeasonsToGrow) {
            lsys.nextGen();
